...
//global items and their meaning/tasks 

struct currentValidID currentValidID; //keeps the current user Valid ID info
CANDIDATE candidateArray[20]; //keeps info of all candiadates 
int numberOfCandidates; //total number of candidates in the poll 
char studentVotes[200]; //keeps votes info from each student 

//extracts the year from iputted userID - eg userID:202020 year:2018
...
int extractRollNO(char userID[15])
{
    ...
    for(int i=9, i<14; i++){
        tmp=userID[i]; 
        rollno=(rollnp*10)+(tmp-48);
    }
    return rollno; 
}

//checks if global branch code and inputed branch code match 
...
void createCandidateFiles(){
    ...
    printf("Crested Files Successfully\n");
}
...
void deleteIllegalVote(char userID[15])
{
    ...
    sprintf(filename, "candidate%d.txt", 
    candidateArray[studentVotes[location-1]-49].cid);
    candidateArray[studentVotes[location-1]-49].votes--;
    studentVotes[location-1]='0';
    if ((fp = fopen(filename, "r"))==NULL)
    {
        printf("\n File Cannote Be Opened...Operation Failed");
        return; 
    }
    printf("\n Deleting in Process...\n");
...
void initialteNewElection()
{
    printf("\n New Election Intitiation: \n"); 
...
void loadElectionInfoFromFile()
{
    ...
    //used to load the candidates information and student votes 
    for (int i = 0; i < currentValidID.totalVoters; i++)
    {
        studentVotes[i] = '0';
    }
    ...
    //used to load the banned/cancelled 
     int location;
